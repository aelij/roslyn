    ...
    void Method()
    {
        [|int x = 0, y = 0; string s;|]
                    ...
                    using (var workspace = await CreateWorkspaceFromFileAsync(source, parseOptions: null, compilationOptions: null))
                    {
                        ...
                        var allFixes = (await fixService.GetFixesAsync(document, span, includeSuppressionFixes: true, foregroundTaskScheduler: TaskScheduler.Default, cancellationToken: CancellationToken.None))
